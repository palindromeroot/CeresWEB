#!/bin/bash

echo "=== Переключение на HTTPS с переадресацией ==="

# Проверяем наличие SSL сертификата
if [ ! -f "./certbot/conf/live/ceres-tech.ru/fullchain.pem" ]; then
    echo "❌ SSL сертификат не найден!"
    echo "💡 Сначала получите сертификат командой: ./deploy.sh"
    exit 1
fi

echo "✅ SSL сертификат найден"

# Переключаемся на HTTPS конфигурацию с переадресацией
echo "🔄 Проверяем конфигурацию nginx..."

# Проверяем синтаксис конфигурации
echo "🔍 Проверка синтаксиса nginx..."
docker compose exec nginx nginx -t

echo "🔄 Перезапуск nginx..."
docker compose restart nginx

echo "⏳ Ожидание перезапуска..."
sleep 5

echo "🔍 Проверка статуса nginx..."
if docker compose ps nginx | grep -q "Up"; then
    echo "✅ Nginx запущен успешно"
    
    echo "🌐 Тестирование переадресации..."
    echo "HTTP запрос должен переадресовать на HTTPS:"
    
    if curl -I -m 10 "http://ceres-tech.ru" 2>/dev/null | grep -q "301\|302"; then
        echo "✅ HTTP переадресация работает!"
    else
        echo "⚠️  Переадресация может не работать"
    fi
    
    echo ""
    echo "🎉 Переключение завершено!"
    echo "🌐 Проверьте сайт:"
    echo "   HTTP:  http://ceres-tech.ru (должен переадресовать)"
    echo "   HTTPS: https://ceres-tech.ru"
    
else
    echo "❌ Ошибка запуска nginx!"
    echo "📝 Проверьте логи:"
    docker compose logs nginx
fi
